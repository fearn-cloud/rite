...
class IngressGenerationTests(unittest.TestCase):
    def test_route_model_combines_service_and_host_routes_ordered_by_hostname(self):
        ...
        self.assertIn("truenas.fearn.cloud", hostnames)
    ...
    def test_service_tcp_ingress_routes_render_caddy_layer4_routes(self):
        model = load_inventory_tree(REPO_ROOT)
    ...
    def test_route_model_requires_unambiguous_static_ipv4_addresses(self):
        ...
        nas_model = deepcopy(load_inventory_tree(REPO_ROOT))
        del nas_model.nas_endpoints["truenas"]["management_address"]
        cases.append(("nas", nas_model, "NAS Ingress Route truenas"))
    ...
    def test_service_ingress_routes_render_distinct_caddy_routes_and_dns_records(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            shutil.copytree(FIXTURES / "inventory_valid", root, dirs_exist_ok=True)
            self._write_internal_ingress(root)
            self._write_service_with_ingress_routes(root)
            (root / "inventory" / "vms" / "dns01.yaml").write_text(
                "vmid: 102\n"
                "placement:\n"
                "  host: wintermute\n"
                "source:\n"
            ...
            (root / "inventory" / "services" / "dns-primary.yaml").write_text(
                self._pihole_dns_target_service_yaml("dns-primary", "dns01")
            )

            model = load_inventory_tree(root)
            route_model = build_caddy_route_model(model)
            caddy_routes = render_caddy_routes(model)
            record_sets = render_ingress_dns_record_sets(model)

            service_routes = [route for route in route_model["routes"] if route["owner"] == "immich"]
            self.assertEqual(
                [
                    ("photos-admin.fearn.cloud", "http://10.0.10.101:8080"),
                    ("photos.fearn.cloud", "http://10.0.10.101:2283"),
                ],
                [(route["hostname"], route["target"]) for route in service_routes],
            )
            self.assertIn("photos.fearn.cloud {\n\ttls", caddy_routes)
            self.assertIn("\treverse_proxy http://10.0.10.101:2283", caddy_routes)
            self.assertIn("photos-admin.fearn.cloud {\n\ttls", caddy_routes)
            self.assertIn("\treverse_proxy http://10.0.10.101:8080", caddy_routes)
            self.assertTrue(record_sets)
            self.assertIn("address=/photos.fearn.cloud/10.40.0.16", record_sets[0]["content"])
            self.assertIn("address=/photos-admin.fearn.cloud/10.40.0.16", record_sets[0]["content"])
    ...
    def test_host_ingress_route_targets_management_address_not_proxmox_endpoint(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("reverse_proxy https://10.0.0.10:8006", caddy_routes)
            self.assertIn("tls_insecure_skip_verify", caddy_routes)
            self.assertNotIn("pve-api.fearn.cloud", caddy_routes)
            self.assertNotIn("pve-internal-name.fearn.cloud", caddy_routes)

    def test_nas_ingress_route_is_limited_to_inventory_trusted_source_ranges(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            shutil.copytree(FIXTURES / "inventory_valid", root, dirs_exist_ok=True)
            self._write_internal_ingress(root)
            ...
            self.assertIn("truenas.fearn.cloud {", caddy_routes)
            self.assertIn("@trusted remote_ip 10.20.0.0/24 100.64.0.0/10", caddy_routes)
            self.assertIn(
                "handle @trusted {\n"
                "\t\treverse_proxy http://10.0.0.15:80\n"
                "\t}",
                caddy_routes,
            )
            self.assertIn("respond 403", caddy_routes)

    def test_nas_ingress_route_upstream_scheme_and_port_are_independent_from_api_tls(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            shutil.copytree(FIXTURES / "inventory_valid", root, dirs_exist_ok=True)
            self._write_internal_ingress(root)
            ...
            self.assertIn("reverse_proxy https://10.0.0.15:8443", caddy_routes)
            self.assertNotIn("tls_insecure_skip_verify", caddy_routes)
    ...
    def test_ingress_regenerate_pushes_generated_files_and_reloads_targets_in_order(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                [
                    "vm-shell internal-ingress-vm -- sudo install -D -m 0644 /dev/stdin /etc/caddy/fortress/generated-layer4.caddy",
                    "stdin: layer4 {",
                    ...
                    "dns-proof @10.40.0.11 30 hostnames",
                    ...
                    "dns-proof @10.40.0.18 30 hostnames",
                ],
                calls,
            )
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                [
                    ...
                    "running: push Caddy layer4 routes to internal-ingress-vm",
                    "ok: push Caddy layer4 routes to internal-ingress-vm",
                    ...
                    "running: prove Ingress DNS records for dns-primary through 10.40.0.11",
                    "ok: prove Ingress DNS records for dns-primary through 10.40.0.11",
                    ...
                    "running: prove Ingress DNS records for dns-secondary through 10.40.0.18",
                    "ok: prove Ingress DNS records for dns-secondary through 10.40.0.18",
    ...
    def test_dns_services_are_web_ui_routes_and_ingress_dns_targets(self):
        ...
        self.assertIn(
            {"bind": "0.0.0.0", "host": 8080, "container": 80, "protocol": "tcp"},
            published_ports,
        )
    ...
    def test_ingress_dns_record_set_renders_declared_routes_to_ingress_address_only(self):
        ...
        self.assertEqual(
            [
                ...
                "address=/clonarr.fearn.cloud/10.40.0.16",
                "address=/directory.fearn.cloud/10.40.0.16",
    ...
    def test_ingress_regenerate_validates_inventory_before_pushes(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("Service Ingress Route", result.stderr)
    ...
    def test_ingress_regenerate_exits_nonzero_when_layer4_push_fails(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            shutil.copytree(REPO_ROOT / "inventory", root / "inventory")
            calls_log = root / "calls.log"
            ...
            result = self._run_ingress_regenerate(root, fail_on=GENERATED_CADDY_LAYER4_ROUTES_PATH)

            self.assertNotEqual(result.returncode, 0)
            self.assertIn("failed: push Caddy layer4 routes to internal-ingress-vm", result.stderr)
            self.assertNotIn(GENERATED_CADDY_ROUTES_PATH, calls_log.read_text())

    def test_ingress_regenerate_exits_nonzero_when_dns_records_do_not_resolve(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            shutil.copytree(REPO_ROOT / "inventory", root / "inventory")
            calls_log = root / "calls.log"
            ...
            result = self._run_ingress_regenerate(
                root,
                extra_env={
                    "FORTRESS_FAKE_DNS_PROOF_FAIL": "record not loaded yet",
                    "FORTRESS_DNS_PROOF_TIMEOUT_SECONDS": "0",
                    "FORTRESS_DNS_PROOF_INTERVAL_SECONDS": "0",
                },
            )

            self.assertNotEqual(result.returncode, 0)
            self.assertIn(
                "failed: prove Ingress DNS records for dns-primary through 10.40.0.11: record not loaded yet",
                result.stderr,
            )
            self.assertNotIn(
                "vm-shell dns-secondary-vm -- sudo install",
                calls_log.read_text(),
            )
    ...
    def _write_service_with_ingress_routes(self, root):
        (root / "inventory" / "services" / "immich.yaml").write_text(
            ...
            "      type: none\n"
            "  - name: admin\n"
            "    hostname: photos-admin.fearn.cloud\n"
            "    published_port: 8080\n"
            "    exposure: lan_only\n"
            "    tls: letsencrypt_dns\n"
            "    auth:\n"
    ...
    def _run_ingress_regenerate(self, root, fail_on="", extra_env=None):
        env = {
