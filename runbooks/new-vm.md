...
1. Prepare generates the per-VM SSH keypair when VM SSH material has not been minted yet. Prepare refuses to run when inventory/vms/<vm>.sops.yaml already contains VM SSH key material, because that means VM private key material has already been minted. In that case `vm-up` runs `vm-prepare-satisfied`, which requires the Inventory public key to match the encrypted Sibling SOPS public key and checks private-key field presence without decrypting it. A pre-existing VM Sibling SOPS File may contain other VM credentials, such as a Tailnet Auth Key; Prepare decrypts, merges the generated SSH key, and re-encrypts it.
