...
Completed the Service Update workflow. The new workflow plan starts with Service Deploy, confirms before restart unless auto-confirmed, restarts only the named Service's declared Quadlet or Native systemd units, verifies each unit reaches active state, and exposes `scripts/service-update` plus `just service-update`. Focused Service Update, Service Launch, and Operator Workflow Runner tests pass. The full foundation runner passed once during implementation, then later failed in concurrent `host_update` work (`build_host_update_plan(..., reboot=True)`), outside this issue's files.
