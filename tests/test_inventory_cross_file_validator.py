...
class InventoryCrossFileValidatorTests(unittest.TestCase):
    ...
    def test_valid_nas_ingress_route_uses_endpoint_name_and_management_address(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            shutil.copytree(FIXTURES / "inventory_valid", root, dirs_exist_ok=True)
            (root / "inventory" / "group_vars" / "all.yaml").write_text(
    ...
    def test_nas_ingress_route_hostname_must_match_endpoint_domain_name(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            shutil.copytree(FIXTURES / "inventory_valid", root, dirs_exist_ok=True)
            (root / "inventory" / "group_vars" / "all.yaml").write_text(
    ...
    def test_nas_ingress_route_hostname_alone_does_not_enable_route(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            shutil.copytree(FIXTURES / "inventory_valid", root, dirs_exist_ok=True)
            (root / "inventory" / "nas" / "truenas.yaml").write_text(
    ...
    def test_nas_ingress_route_hostname_shares_service_and_host_ingress_namespace(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            shutil.copytree(FIXTURES / "inventory_valid", root, dirs_exist_ok=True)
            (root / "inventory" / "group_vars" / "all.yaml").write_text(
    ...
    def test_nas_ingress_route_requires_management_address_target(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            shutil.copytree(FIXTURES / "inventory_valid", root, dirs_exist_ok=True)
            (root / "inventory" / "group_vars" / "all.yaml").write_text(
