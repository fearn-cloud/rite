...
class UpdateWorkflowRunbookTests(unittest.TestCase):
    def test_runbook_documents_scoped_update_workflows(self):
        ...
        expected_phrases = [
            ...
            "checks whether the Host reports `/var/run/reboot-required`",
            "newest installed `/boot/vmlinuz-*` kernel",
            "differs from the running kernel",
            "`needrestart` findings",
            "`checkrestart` findings",
            "stops without interrupting VMs",
            "the reboot flag is the explicit maintenance-window approval for Host Update",
            "waits for reboot completion and Host reachability",
